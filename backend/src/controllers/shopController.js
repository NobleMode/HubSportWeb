...
class ShopController {
  ...
  async requestWithdrawal(req, res, next) {
    try {
      ...
      if (
        ...
        error.message === "Minimum withdrawal amount is 50,000 VND"
      ) {
  ...
  async getWithdrawalHistory(req, res, next) {
    try {
      ...
      const withdrawals = await shopService.getWithdrawalHistory(shop.id);
